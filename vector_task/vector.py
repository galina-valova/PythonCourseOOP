...
class Vector:
    def __init__(self, *args):
        arguments_number = len(args)

        if arguments_number > 2 or arguments_number == 0:
            raise ValueError(f"Invalid number of arguments, your input is {arguments_number}")

        if arguments_number == 1 and isinstance(args[0], int):
            vector_size = args[0]
        ...
        if arguments_number == 1 and isinstance(args[0], Vector):
            vector = args[0]
            self.__components = list(vector.__components)
            return

        if arguments_number == 1 and isinstance(args[0], list):
            ...
            if not all(isinstance(item, (int, float)) for item in components_list):
                raise TypeError(f"Vector components must be numbers, not",
                                [type(item).__name__ for item in components_list])

            self.__components = list(components_list)
            return

        if arguments_number == 2 and isinstance(args[0], int) and isinstance(args[1], list):
            vector_size = args[0]
    ...
    def __iadd__(self, other):
        ...
        if self.size < other.size:
            ...
            for i in range(min_vector_size, other.size):
                self.__components.append(other.__components[i])
